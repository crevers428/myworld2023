...
class Schedule
{
	...
	public function out(&$py,$pm,$IE)
	{
		...
		//
		$ncolor = 0;
		$colors = array("#7381FB;","#FF939D;","#FFF977;","#85E377;","#FFA74F;","#f0f0f0;");
		//
		$echo  = "\r\n";
		$x = 0;
		while ($y < 288)
		{
				...
				(!isset($current) || !array_key_exists($x,$current) || $current[$x]!=$this->sectors[$x][$y]))
			{
				...
				if (array_key_exists($evt,$this->categories) || $evt=="reg" || $evt=="tro" || $evt=="lun")
					$echo .= "<img border=0 src='{{file}}img/".$evt.".gif' style='position:absolute;top:".$img_y."px;left:-5px;'>";
				//
